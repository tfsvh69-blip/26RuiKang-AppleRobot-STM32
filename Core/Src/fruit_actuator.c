...
/* 分类舵机大果仓方向：大果抓取完成后转向此角度再松开夹爪。 */
#define FRUIT_ACTUATOR_SORTER_BIG_DEG       270.0f
/* 分类舵机小果仓方向：小果抓取完成后转向此角度再松开夹爪。 */
#define FRUIT_ACTUATOR_SORTER_SMALL_DEG     0.0f
/* 大果篮装载角度：比赛过程中接收大果的正常姿态。 */
#define FRUIT_ACTUATOR_BIG_BASKET_LOAD_DEG  135.0f
/* 大果篮倒出角度：最终把大果从暂存篮倒出去的姿态。 */
#define FRUIT_ACTUATOR_BIG_BASKET_DUMP_DEG  90.0f
/* 小果篮装载角度：比赛过程中接收小果的正常姿态。 */
#define FRUIT_ACTUATOR_SMALL_BASKET_LOAD_DEG 125.0f
/* 小果篮倒出角度：最终把小果从暂存篮倒出去的姿态。 */
...
/*
 * 分类舵机转到大果/小果仓方向后，等待多久再打开夹爪，单位 ms。
 *
...
static int FruitBasket_DumpTwice(servo_id_e servo_id,
...
/**
  * @brief 大果暂存篮倒出苹果。
  * @note  SERVO_4 先从装载角度 135 度转到倒出角度 90 度，
  *        等待果子滑出后再回到 135 度；整套动作会执行两次，
...
{
  return FruitBasket_DumpTwice(FRUIT_ACTUATOR_BIG_BASKET_SERVO,
...
/**
  * @brief 小果暂存篮倒出苹果。
  * @note  SERVO_5 先从装载角度 125 度转到倒出角度 170 度，
...
{
  return FruitBasket_DumpTwice(FRUIT_ACTUATOR_SMALL_BASKET_SERVO,
